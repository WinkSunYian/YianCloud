...
class RobberyService:
    ...
    @staticmethod
    async def calculate_robbery(user_id: int, target_id: int, item_name: str):
        """
        ...
        if not target:
            return {
                "message": f"{target_id}的背包里没有{item_name}",
            }

        user_item = await ItemService.get_by_user_id_and_name(user_id, item_name)
        if not user_item:
            pass

        target_item = await ItemService.get_by_user_id_and_name(target_id, item_name)
        if not target_item:
            return {
                "message": f"{target_id}的背包里没有{item_name}",
            }
        ...
        # 根据目标道具的数量来确定打劫数量的范围
        min_robbery_quantity = -min(10, user_item.quantity)
        ...
        if robbery_quantity > 0:
            # 减少目标道具数量，增加打劫者道具数量
            await ItemService.reduce_quantity(target_id, item_name, robbery_quantity)
            await ItemService.add_quantity(user_id, item_name, robbery_quantity)
            ...
            await ItemService.reduce_quantity(user_id, item_name, -robbery_quantity)
            return {
