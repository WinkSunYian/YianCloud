...
def register_router(
    router, app: FastAPI, root: str, directory: str, router_path
) -> FastAPI:
    ...
    app.include_router(
        router, prefix=f"/{full_router_path.strip('/')}".replace("\\", "/")
    )
    return app


def auto_register_routes(
    app: FastAPI, router_path: Optional[str] = None, directory: str = "src/"
):
        ...
        for file in files:
            if file.endswith(".py") and file != "__init__.py":
                ...
                for attr in dir(module):
                    ...
                    if (
                        isinstance(cls, type)
                        and issubclass(cls, ServiceRouter)
                        and cls is not ServiceRouter
                    ):
                        instance = cls()
                        register_router(
                            instance.router, app, root, directory, router_path
                        )
